 ...
 * @param team
 */
function getIdFromMatchInfo(match: number, category: MatchType, team: number): string {
...
export async function writeMatch(data: MatchInfo): Promise<boolean> {
	if (data.match === undefined || data.matchCategory === undefined || data.team === undefined) {
		return false;
	}
 ...
 * @param team
 */
export async function readMatch(match: number, matchCategory: MatchType, team: number): Promise<MatchInfo> {
	return JSON.parse(await AsyncStorage.getItem(getIdFromMatchInfo(match, matchCategory, team)) || "null");
}
